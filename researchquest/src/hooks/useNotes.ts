...
export function useNotes(userId: string | undefined) {
  ...
  const syncSelectedNote = useCallback(
    (updated: Note | null) => {
      if (!updated) return;
  ...
  const createNote = useCallback(
    async (noteData: Partial<Note>): Promise<Note | null> => {
      if (!userId) {
    ...
    async (noteId: string, updates: Partial<Note>): Promise<boolean> => {
      ...
      if (updateError) {
        ...
        if (originalNote) {
          setNotes(
            sortByUpdatedAt(
              freshNotes.map((n) => (n.id === noteId ? originalNote : n)),
            ),
          );
  ...
  const restoreNote = useCallback(
    async (note: Note): Promise<Note | null> => {
      ...
      setNotes(
        sortByUpdatedAt([
          restoredNote,
          ...currentNotes.filter((existing) => existing.id !== restoredNote.id),
        ]),
      );
