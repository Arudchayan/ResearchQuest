...
export function useDataSync(userId: string | undefined) {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            setNotes(dedupeById([payload.new as Note, ...useAppStore.getState().notes]))
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setPapers(sortByUpdatedAt([newPaper, ...useAppStore.getState().papers]))
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
             ...
             const currentIdeas = useAppStore.getState().ideas
             setIdeas(currentIdeas.map(i => i.id === updated.id ? updated : i))
  ...
  }, [userId, setNotes, setPapers, setIdeas, setNotesLoading, setPapersLoading, setIdeasLoading, setSelectedPaper, setSelectedIdea])
}
