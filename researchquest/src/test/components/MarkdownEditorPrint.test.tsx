...
describe('MarkdownEditor Print', () => {
  ...
  it('opens print window with correct content when print button is clicked', async () => {
    ...
    expect(mockWrite).toHaveBeenCalledWith(expect.stringContaining('Test Note Title'))
    expect(mockWrite).toHaveBeenCalledWith(expect.stringContaining('Heading')) // From rendered markdown
    expect(mockWrite).toHaveBeenCalledWith(expect.stringContaining('<strong>bold</strong>')) // Rendered markdown HTML
    ...
    expect(mockWrite).toHaveBeenCalledWith(expect.stringContaining('window.print()'))
  })
})
