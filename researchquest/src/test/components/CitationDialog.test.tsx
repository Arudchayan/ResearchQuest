...
describe("CitationDialog", () => {
  it("renders correctly and defaults to BibTeX", () => {
    ...
    expect(screen.getByRole("tab", { name: "BibTeX" })).toHaveAttribute(
      "data-state",
      "active",
    );
  ...
  it("switches tabs and updates citation format", async () => {
    ...
    expect(
      screen.getByText(/One, A. & Two, A. \(2023\). Test Paper./),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/One, Author, and Author Two "Test Paper." 2023./),
    ).toBeInTheDocument();
  });
