...
vi.mock("../../components/papers/PaperCard", () => ({
  PaperCard: ({ paper }: any) => (
    <div data-testid="paper-card">{paper.title}</div>
  ),
}));
...
describe("ListFiltering Performance", () => {
  const mockNotes = [
    {
      id: "1",
      title: "Note 1",
      markdown_body: "Body content ONE",
      updated_at: new Date().toISOString(),
    },
    {
      id: "2",
      title: "Note 2",
      markdown_body: "Body content TWO",
      updated_at: new Date().toISOString(),
    },
  ];
  ...
  it("should verify optimization: filter is NOT called on notes when search query is empty", () => {
    ...
    render(<NotesView />);
    const calledOnMockNotes = filterSpy.mock.instances.some(
      (instance) => instance === mockNotes,
    );
  ...
  it("should verify optimization: filter is NOT called on papers when search query is empty", () => {
    const filterSpy = vi.spyOn(Array.prototype, "filter");
    render(<PapersView />);
    const calledOnMockPapers = filterSpy.mock.instances.some(
