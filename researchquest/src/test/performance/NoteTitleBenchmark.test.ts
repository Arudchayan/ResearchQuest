...
describe("Performance: Note Title Derivation", () => {
  ...
  it("verifies both implementations produce the same output", () => {
    expect(deriveTitleFromMarkdown(shortNote)).toBe(
      deriveNoteTitleOld(shortNote),
    );
    expect(deriveTitleFromMarkdown(longNote)).toBe(
      deriveNoteTitleOld(longNote),
    );
    expect(deriveTitleFromMarkdown(massiveNote)).toBe(
