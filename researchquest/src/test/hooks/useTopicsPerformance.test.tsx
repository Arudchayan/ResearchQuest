...
describe("useTopics Performance", () => {
  ...
  it("should cache entity links on remount (showing optimization)", async () => {
    const linkSelectSpy = vi.fn().mockReturnValue(
      createMockBuilder({
        eq: vi.fn().mockReturnValue(
          createMockBuilder({
            then: ((onFulfilled: any) =>
    ...
    const linkFetchCalls = linkSelectSpy.mock.calls.filter(
      (args: any[]) => args[0] === "topic_id",
    );
    ...
    const linkFetchCalls2 = linkSelectSpy.mock.calls.filter(
      (args: any[]) => args[0] === "topic_id",
    );
