...
export const useGamificationStore = create<GamificationState>((set, get) => {
  ...
  return {
    ...
    hydrateFromProfile: (profile) => {
      const freezeTokens = profile.streak_freeze_tokens ?? get().streakFreezeTokens
      const restDays = profile.rest_days ?? get().restDays
      const activeBoost = (profile.active_boost as ActiveBoost | null | undefined) ?? null
    ...
    activateBoost: async (userId, config) => {
      const expiresAt = new Date(Date.now() + config.durationMinutes * 60 * 1000).toISOString()
