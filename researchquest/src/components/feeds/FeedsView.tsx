  ...
  return (
    <div className="min-h-full bg-bg-base px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        <header className="rounded-surface border border-border-moderate bg-bg-surface p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex items-center gap-3 text-text-primary">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-primary-100 text-text-primary">
                  <Inbox className="h-5 w-5" aria-hidden="true" />
                </div>
                <div>
                  <p className="mb-1 text-caption font-semibold uppercase tracking-wide text-text-tertiary">Research intelligence</p>
        ...
        <section
          className="rounded-surface border border-border-moderate bg-bg-surface p-4 shadow-sm sm:p-5"
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {(["all", ...FEED_ITEM_TYPES] as FeedTypeFilter[]).map((filter) => (
                  <button
                    ...
                    className={cn(
                      "rounded-control border px-3 py-1.5 text-small font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus",
                      type === filter
          ...
          {loading ? (
            <div className="space-y-3" role="status" aria-live="polite">
              {[0, 1, 2].map((index) => (
                <div
                  key={index}
                  className="h-44 animate-pulse rounded-surface border border-border-moderate bg-bg-surface shadow-sm"
                />
              ))}
              <span className="sr-only">Loading feeds</span>
            </div>
          ) : error ? (
            <div className="rounded-surface border border-border-moderate bg-bg-surface p-6 text-text-secondary shadow-sm">
              {error}
            </div>
          ) : items.length === 0 ? (
            <div
              role="status"
              aria-live="polite"
              className="rounded-surface border border-dashed border-border-moderate bg-bg-surface p-10 text-center shadow-sm"
            >
              <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-control bg-bg-elevated text-text-tertiary">
                <Inbox className="h-5 w-5" aria-hidden="true" />
              </span>
