...
export function useEditorActions(
  ...
  const handlePrint = useCallback(() => {
    ...
    // Safely insert dynamic content using DOM APIs to prevent XSS
    printWindow.document.title = rawTitle;
    const titleEl = printWindow.document.getElementById("note-title");
    if (titleEl) {
      titleEl.textContent = rawTitle;
    }
    const contentEl = printWindow.document.getElementById("note-content");
    if (contentEl) {
