...
import { Pencil, Save, XCircle, FileText, BookOpen, Lightbulb, ArrowRight, Trash2 } from 'lucide-react'

interface TopicDetailViewProps {
  topic: TopicWithCounts
  onUpdate: (topicId: string, updates: { name?: string; description?: string }) => Promise<boolean>
  onDelete: (topicId: string) => Promise<boolean>
}

export function TopicDetailView({ topic, onUpdate, onDelete }: TopicDetailViewProps) {
  const {
    setCurrentView,
    setSelectedNote,
    setSelectedPaper,
  ...
  const loadAssociations = useCallback(async () => {
    ...
    const fetchIds = async <T extends string>(table: string, column: string): Promise<T[]> => {
  ...
  const handleDelete = async () => {
    if (!confirm('Delete this topic? This will remove its links to notes, papers, and ideas.')) {
      return
    }
  ...
  const associationSummary = useMemo(
    () => [
      { label: 'Notes', count: topic.note_count, items: notes, icon: FileText, view: 'notes' as const },
      { label: 'Papers', count: topic.paper_count, items: papers, icon: BookOpen, view: 'papers' as const },
      { label: 'Ideas', count: topic.idea_count, items: ideas, icon: Lightbulb, view: 'ideas' as const },
    ],
    [ideas, notes, papers, topic.idea_count, topic.note_count, topic.paper_count]
  )
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="flex-1 space-y-2">
            {isEditing ? (
              ...
              <h1 className="text-2xl font-bold text-text-primary">{topic.name}</h1>
            )}
            {isEditing ? (
              <textarea
                value={description}
              ...
              <p className="text-body text-text-secondary whitespace-pre-wrap">
                {topic.description || 'Add a description to guide your future self.'}
              </p>
            )}
          </div>
      ...
      <div className="bg-bg-surface border border-border-subtle rounded-xl shadow-sm">
        <div className="px-6 py-4 border-b border-border-subtle flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-text-primary">Connected work</h2>
            <p className="text-caption text-text-secondary">Jump back into the work linked to this topic.</p>
          </div>
          <button
        ...
        <div className="divide-y divide-border-subtle">
          {associationSummary.map(({ label, count, items, icon: Icon, view }) => (
            <div key={label} className="px-6 py-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Icon className="w-4 h-4 text-primary-500" />
                  <h3 className="text-small font-semibold text-text-primary">
                    {label} <span className="text-text-tertiary font-normal">({count})</span>
                  </h3>
                </div>
                {count > 0 && (
                  <button
              ...
              {loadingAssociations ? (
                ...
                <p className="text-caption text-text-tertiary">No {label.toLowerCase()} linked yet.</p>
              ) : (
                    ...
                    <li key={item.id}>
                      ...
                      >
                        <p className="text-small font-medium text-text-primary line-clamp-1">
                          {'title' in item && item.title ? item.title : label === 'Notes'
