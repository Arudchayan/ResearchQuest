...
export function IdeasOverview({ ideas, loading, onCreate, onSelect }: IdeasOverviewProps) {
  ...
  const activeCount = ideas.filter((idea) => idea.stage === 'Seed' || idea.stage === 'Developing').length

  const ideaHighlights = useMemo(() => {
    if (ideas.length === 0) {
      return {
        headline: 'Start your idea garden',
        detail: 'Capture the sparks that show up during reading and experiments. Name the idea and add a quick description to anchor it.',
      }
    }
    ...
    if (activeCount > 0) {
      return {
        ...
        detail: 'Prioritise the Seed and Developing ideas to maintain momentum. Pull one into focus and outline the next validation step.',
      }
    }

    return {
      headline: 'Ideas ready for synthesis',
      detail: 'Most ideas are in later stages. Schedule time to polish outcomes or publish findings.',
    }
  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col gap-3">
        ...
        <h1 className="text-3xl font-bold text-text-primary">Shape and evolve your research ideas</h1>
        <p className="text-text-secondary max-w-3xl">
          Use the idea board to capture sparks, nurture promising leads, and track which concepts are ready for validation or publication.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {STAGES.map((stageName) => (
          <div key={stageName} className="bg-bg-surface border border-border-subtle rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-text-secondary">{stageName}</span>
              <span className="text-lg font-bold text-text-primary">{stageBuckets[stageName].length}</span>
            </div>
            <p className="text-caption text-text-tertiary mt-1">
              {stageName === 'Seed' && 'Fresh hypotheses waiting for exploration'}
              {stageName === 'Developing' && 'Ideas being validated and connected'}
              {stageName === 'Supported' && 'Concepts with evidence or linked notes'}
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_1fr]">
        <div className="bg-bg-surface border border-border-subtle rounded-2xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-text-primary">Capture a new idea</h2>
              <p className="text-sm text-text-secondary">Log the concept quickly, then evolve it in detail view.</p>
            </div>
          </div>
        ...
        <div className="bg-primary-500/10 border border-primary-500/20 rounded-2xl shadow-inner p-6 space-y-3">
          ...
          <h3 className="text-xl font-semibold text-primary-700 dark:text-primary-300">{ideaHighlights.headline}</h3>
          <p className="text-sm text-primary-700/80 dark:text-primary-200/80 leading-relaxed">{ideaHighlights.detail}</p>
        </div>
      </div>

      <div className="space-y-5">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-2xl font-semibold text-text-primary">Idea board</h2>
          <p className="text-sm text-text-secondary">Click an idea card to dive deeper, link notes, or update the stage.</p>
        </div>
        <div className="grid gap-4 lg:grid-cols-4">
          {STAGES.map((stageName) => (
            <div key={stageName} className="bg-bg-surface border border-border-subtle rounded-2xl shadow-sm p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-text-secondary">{stageName}</span>
                <span className="text-caption text-text-tertiary">{stageBuckets[stageName].length} item{stageBuckets[stageName].length === 1 ? '' : 's'}</span>
              </div>
              <div className="space-y-3">
                {stageBuckets[stageName].length === 0 ? (
                  <div className="text-caption text-text-tertiary bg-bg-base/60 border border-border-subtle/60 rounded-lg p-3">
                    {stageName === 'Seed' ? 'Capture a new idea to plant a seed.' : 'No entries yet. Promote an idea when it reaches this stage.'}
                  </div>
                ) : (
                  stageBuckets[stageName].slice(0, 4).map((idea) => (
                    ...
                    >
                      ...
                      <p className="text-caption text-text-tertiary mt-1">Updated {formatUpdatedAt(idea.updated_at)}</p>
                      <p className="text-caption text-text-secondary mt-2 line-clamp-3">{ideaDescriptionPreview(idea.description)}</p>
