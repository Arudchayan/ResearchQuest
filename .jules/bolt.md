...
## 2025-04-20 - Mismatched Sort Defaults and localeCompare Correctness
**Learning:** If an early return in a `useMemo` filter block relies on a default `sortOption` condition that doesn't match the actual default state (e.g., checking for `"name_asc"` when the default is `"updated_desc"`), the fast path is never hit, causing an unnecessary array copy and O(N log N) sorting operation on every initial render. Also, while `localeCompare` is slower than direct string comparison (`a > b`), removing it for user-facing alphabetical sorts (like names or titles) breaks case-insensitivity and locale awareness, violating correctness.
**Action:** Always ensure that `useMemo` early return conditions exactly match the initial component state to preserve fast paths. Never remove `localeCompare` for user-facing text sorting just for a performance micro-optimization, as correctness (case and locale handling) is more important.
