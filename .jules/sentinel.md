...
## 2025-05-18 - URL Validation Bypass using Control Characters
**Vulnerability:** The application's `isValidUrl` function checked for safe protocols by performing standard parsing or string prefix matching. It did not properly account for non-printable ASCII control characters (e.g., `\x00`) or zero-width spaces (e.g., `\u200B`) being inserted into the URL. For instance, the string `\x00javascript:alert(1)` might be parsed inconsistently depending on the library but is generally stripped out or ignored by browsers when inserted into an `href` attribute, resulting in Cross-Site Scripting (XSS).
**Learning:** Checking prefixes or relying purely on basic parser implementations without prior sanitization of control characters allows attackers to bypass security checks for malicious schemas. Browsers will often sanitize and execute the obfuscated payload.
**Prevention:** Always sanitize inputs meant to be URLs by explicitly stripping out ASCII control characters and zero-width characters (e.g., via `url.replace(/[\x00-\x1F\x7F-\x9F\u200B-\u200D\uFEFF]/g, "")`) before performing URL validation or schema checking.
